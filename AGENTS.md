...
- For `web/views`, keep the body focused on layout usage and shared-component usage. Document which layout slots are filled, how the view maps values into each shared-component input, and any route guard or entry policy the view depends on. Keep route path in front matter.
